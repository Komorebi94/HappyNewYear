...
export function calcRepScheme (sessionIndex, {
    baseReps,
    baseSets,
    repCap,
    setCap = 5,
    ...
    const rhythmNote =
        phase === 'accumulate' ? `本周期第 ${pos + 1} 天，+${repStep} 次` :
            phase === 'consolidate' ? '维持次数，打磨动作质量' :
                sets > baseSets ? '升阶：加 1 组' : `升阶：+${repStep} 次`

    return { reps, sets, phase, rhythmNote }
}

export function calcHoldScheme (sessionIndex, {
    baseSec,
    baseSets,
    secCap,
    setCap = 4,
    ...
    const rhythmNote =
        phase === 'accumulate' ? `本周期第 ${pos + 1} 天，+${holdStep} 秒` :
            phase === 'consolidate' ? '维持秒数，核心收紧' :
                sets > baseSets ? '升阶：加 1 组' : `升阶：+${holdStep} 秒`
...
export function getUnlockedPushVariants (sessionIndex) {
    ...
    return getEarnedPushVariants(sessionIndex).filter(
        (v) => variantOrder(v.id) >= recOrder
    )
}
