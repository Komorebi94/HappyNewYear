...
export function getTodayPlan (totalCheckDays, selectedPushId) {
    ...
    tasks.push(buildExerciseTask(secondary, sessionIndex, secondary.id === 'wall_angle' ? '肩背' : '核心'))

    const finisherPool = sessionIndex >= 12
        ? FINISHER_EXERCISES
        : FINISHER_EXERCISES.filter((f) => f.id === 'stretch')
    const finisher = finisherPool[sessionIndex % finisherPool.length]
    if (finisher) {
        tasks.push(buildExerciseTask(finisher, sessionIndex, finisher.id === 'stretch' ? '放松' : '有氧激活'))
    }
