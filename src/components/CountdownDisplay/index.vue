    ...
    >
        ...
        <Transition name="fade" mode="out-in">
            ...
            <div v-else key="celebration" class="new-year-message">
                <div
                    ref="messageBoxRef"
                    class="new-year-message__box"
                >
                    <span
                        v-for="(char, index) in newYearMessage"
                        :key="index"
                        class="char"
                    >{{ char }}</span>
                </div>
...
watch(countdownNumberRef, (el) => {
    emit('countdown-number-ref', el)
}, { immediate: true })

watch(messageBoxRef, (el) => {
...
@media (max-width: $mobile-max) {
    .countdown-cards { gap: 0.35rem; }
    .card-value { font-size: 1.35rem; }
    .block-value { font-size: 2.25rem; }
    .fullscreen-countdown h1 { font-size: clamp(5rem, 22vw, 9rem); }
}

@media (min-width: $tablet-min) and (max-width: $tablet-max) {
    .countdown-cards { gap: 0.75rem; }
    .card-value { font-size: 1.75rem; }
    .block-value { font-size: 2.5rem; }
    .fullscreen-countdown h1 { font-size: clamp(4.5rem, 14vw, 7rem); }
}

@media (min-width: $desktop-min) {
    .countdown-cards { gap: 1rem; }
    .card { padding: 1rem 0.5rem; }
    .card-value { font-size: 2rem; }
    .block-value { font-size: 2.75rem; }
    .fullscreen-countdown h1 { font-size: clamp(5rem, 10vw, 8rem); }
}
